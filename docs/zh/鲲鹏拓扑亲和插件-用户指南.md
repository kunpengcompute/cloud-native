...
- Kunpeng TAP与K8s开源版本的拓扑管理器尚不兼容，在部署该插件之前需确认K8s开源版本的拓扑管理器的状态为关闭状态（默认为关闭状态）。可通过查看Kubelet命令行中_--topology-manager-policy_为**关闭**状态即可。
...
1. 删除Kubelet参数。
    ...
    将添加的参数_--docker-endpoint=unix:///var/run/kunpeng/tap-runtime-proxy.sock_删除，并且重启Kubelet。
