...
| AC-ID | 验收标准 | 对应需求 |
|-------|----------|----------|
| AC-01 | 在支持 Cluster 拓扑的新机型上启用 `--topology-cluster-affinity` 后，4 核容器分配在单个 Cluster 内 | FR-CFG-01, FR-ALLOC-01 |
| AC-02 | 在新机型上，20 核容器（> Cluster 大小）分配在单个 NUMA 内 | FR-ALLOC-02 |
| AC-03 | 在新机型上，50 核容器（> NUMA 大小）分配在单个 Socket 内 | FR-ALLOC-03 |
| AC-04 | 不支持 Cluster 拓扑的机型上启用 Cluster 亲和后，功能自动禁用，回退到 NUMA 级别分配 | FR-DET-03 |
| AC-05 | Cluster 资源不足时，容器能够分配到其他 Cluster | FR-ALLOC-05 |
| AC-06 | 系统能够正确识别 SMT 开启和关闭两种配置 | FR-TOPO-02, NFR-03 |
| AC-07 | Prometheus 指标能够正确统计 Cluster 级别分配的计数 | FR-MON-01 |
