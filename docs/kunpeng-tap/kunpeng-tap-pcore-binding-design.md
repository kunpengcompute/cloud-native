# Kunpeng-TAP Pcore Binding 功能设计说明

| 标题 | Kunpeng-TAP Pcore Binding 功能设计说明 |
| --- | --- |
| 状态 | Draft |
| 作者 | Kunpeng Boostkit Team |
| 创建日期 | 2026-07-16 |
| 项目名称 | `kunpeng-tap-pcore-binding` |
| 目标场景 | Kata / Confidential Containers Pod 级 cpuset 收敛 |

## 1. 功能概述

`kunpeng-tap-pcore-binding` 面向基于 Kata 的机密容器运行场景。插件通过 NRI 接入 containerd，在 Pod sandbox 生命周期事件和周期扫描中，对符合白名单条件的 Kata Pod 执行 Pod 级 cpuset 收敛，将 Pod 的 2 个逻辑 CPU 绑定到同一个物理核心的 SMT sibling pair。
...
flowchart TB
    subgraph K8S["Kubernetes Worker Node"]
        ...
        Plugin["kunpeng-tap-pcore-binding"]
...
```mermaid
flowchart LR
    Main["cmd/kunpeng-tap-pcore-binding<br/>main.go"]
    Agent["pkg/kunpeng-tap-pcore-binding/plugin<br/>Agent"]
    Topology["pkg/kunpeng-tap-pcore-binding/topology<br/>SiblingPair discovery"]
...
| 模块 | 路径 | 职责 |
| --- | --- | --- |
| 进程入口 | `cmd/kunpeng-tap-pcore-binding/main.go` | 解析 CLI 参数、发现 sibling pair、创建 Agent、处理退出信号。 |
| NRI Agent | `pkg/kunpeng-tap-pcore-binding/plugin/agent.go` | 实现 NRI 回调、维护 Pod 元数据缓存、调度调和循环、解析 cgroup 并写入 cpuset。 |
| 拓扑发现 | `pkg/kunpeng-tap-pcore-binding/topology/siblings.go` | 读取 CPU sysfs sibling 信息，生成稳定排序的 sibling pair 列表。 |
| 部署清单 | `config/kunpeng-tap-pcore-binding/daemonset.yaml` | 定义 DaemonSet、挂载、启动参数和安全上下文。 |
...
插件通过二进制 `kunpeng-tap-pcore-binding` 启动，主要参数如下：
...
- 查看插件日志中是否出现 `Using kunpeng tap pcore binding config`、`Pod cpuset updated` 或 `Dry-run pod cpuset update`。
