...
@given(parsers.parse('集群有部署 {daemonset} daemonset配置的节点'))
def cluster_has_daemonset(daemonset):
    """验证集群具有指定的机器配置 """ 
    test_context['daemonset'] = daemonset 
    # 验证集群节点是否符合配置要求 
    daemonsets = k8s_apps_v1.list_namespaced_daemon_set("default") 
    haveDaemonset = False 
    for ds in daemonsets.items: 
        if ds.metadata.name == daemonset: 
            haveDaemonset = True 
            break 
    if haveDaemonset == False: 
        raise Exception(f"集群中没有: {daemonset} daemonset") 
    print(f"✅ 集群有{daemonset} daemonset, 验证通过")
...
@then(parsers.parse('容器的KAE QoS应该被成功设置 {devType} 为 {qos_value}'))
def verify_kae_qos(devType, qos_value):
    pod = _get_test_pods()
    ...
    resp = stream(
        ...
        name=pod_name,
        namespace="default",
        command=["/bin/sh", "-c", "env | grep HISI"],
        stderr=True,
        stdin=False,
        stdout=True,
        tty=False
    )
    pci_addr = get_pci_addr(devType, resp)
    set_qos_value = get_qos_value(devType, pci_addr)
    if set_qos_value != qos_value:
        raise Exception(f"{devType} qos set error,excepted {qos_value}, but {set_qos_value}")
    print(f"✅ {devType} qos set success")


# ============================================================================
# 辅助函数 - 获取KAE QoS
# ============================================================================
def get_pci_addr(devType, hisi_env):
    """
    从环境变量字符串中解析指定设备类型的 PCI 地址。
    
    参数:
        devType (str): 设备类型（如 'hpre', 'zip', 'sec2'）
        hisi_env (str): 含有多行 HISI_* 的字符串

    返回:
        str: 匹配的 PCI 地址，例如 '0000:3e:00.3'
        None: 如果没找到匹配项
    """
    # 将输入统一为大写匹配格式，例如 'hpre' -> 'HISI_HPRE'
    pattern_prefix = f"HISI_{devType.upper()}"

    # 遍历每一行
    for line in hisi_env.strip().splitlines():
        line = line.strip()
        # 用正则匹配格式类似 "HISI_HPRE-7=0000:3e:00.3"
        match = re.match(rf"^{pattern_prefix}-\d+=(\S+)$", line)
        if match:
...
def get_qos_value(devType, pci_addr):
    """
    根据设备类型和 PCI 地址读取 QoS 值。

    参数:
        devType (str): 设备类型（如 'hpre', 'zip', 'sec2'）
        pci_addr (str): 设备 PCI 地址（如 '0000:3e:00.3'）

    返回:
        str: alg_qos 文件内容（去掉换行）
        None: 如果文件不存在或读取失败
    """
    # 拼接目录路径，例如：
    # /sys/kernel/debug/hisi_hpre/0000:3e:00.3/alg_qos
    base_path = f"/sys/kernel/debug/hisi_{devType.lower()}/{pci_addr}"
    qos_path = os.path.join(base_path, "alg_qos")
    print(qos_path)
    ...
    try:
        ...
        print(f"[WARN] 文件不存在: {qos_path}")
    except PermissionError:
        print(f"[WARN] 无权限读取: {qos_path}")
