...
func init() {
	err := flag.Set("v", "4") //Enable debug output
	if err != nil {
		fmt.Printf("set flag v4 failed: %v\n", err)
	}
}
...
func (k *kubeletStub) start() error {
	err := os.Remove(k.socket)
	if err != nil {
		klog.Errorf("Remove socket failed: %v", err)
	}
...
func TestSetupAndServe(t *testing.T) {
	...
	err = os.Remove(path.Join(devicePluginPath, pEndpoint))
	if err != nil {
		klog.Errorf("Remove plugin socket failed: %v", err)
	}
