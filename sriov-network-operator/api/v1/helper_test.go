...
func TestNetFilterType_String(t *testing.T) {
	tests := []struct {
		name     string
		input    v1.NetFilterType
		expected string
	}{
		{
			name:     "OpenstackNetworkID",
			input:    v1.OpenstackNetworkID,
			expected: "openstack/NetworkID",
		},
		{
			name:     "QemuKvmNetworkID",
			input:    v1.QemuKvmNetworkID,
			expected: "qemu-kvm/NetworkID",
		},
		{
			name:     "UnknownType",
			input:    v1.NetFilterType(99),
			expected: "99",
		},
	}
