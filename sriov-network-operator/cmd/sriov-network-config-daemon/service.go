...
func (s *ServiceConfig) getPlugin(phase string) (plugin.VendorPlugin, error) {
	...
	case consts.VirtualOpenStack, consts.VirtualQemuKVM:
		switch phase {
		case PhasePre:
	...
	default:
		return nil, fmt.Errorf("unknown platform type %s", s.sriovConfig.PlatformType)
	}
...
func (s *ServiceConfig) getNetworkNodeState(phase string) (*sriovv1.SriovNetworkNodeState, error) {
	...
	case consts.VirtualQemuKVM:
		platformHelper, err := newPlatformHelperFunc()
		if err != nil {
			return nil, fmt.Errorf("failed to create platformHelpers")
		}
		err = platformHelper.CreateQemuKvmDevicesInfo()
		if err != nil {
			return nil, fmt.Errorf("failed to read QEMU-KVM data: %v", err)
		}
		ifaceStatuses, err = platformHelper.DiscoverSriovDevicesVirtualInQemuKvm()
		if err != nil {
			return nil, fmt.Errorf("failed to discover devices: %v", err)
		}
	}
