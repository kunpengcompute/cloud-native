...
func (k *kernel) CheckRDMAEnabled() (bool, error) {
	...
	_, stderr, moduleErr := k.utilsHelper.RunCommand("/bin/sh", "-c", fmt.Sprintf("%s lsmod | grep  -E --quiet 'mlx5_core|hinic'", chrootDefinition))
	if moduleErr != nil && len(stderr) != 0 {
		log.Log.Error(moduleErr, "CheckRDMAEnabled(): failed to check for kernel module 'mlx5_core|hinic'", "stderr", stderr)
		return false, fmt.Errorf(stderr)
	}
