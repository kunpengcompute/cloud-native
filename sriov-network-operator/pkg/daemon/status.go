...
func (dn *NodeReconciler) updateStatusFromHost(nodeState *sriovnetworkv1.SriovNetworkNodeState) error {
	...
	if vars.PlatformType == consts.VirtualQemuKVM {
		ifaces, err = dn.platformHelpers.DiscoverSriovDevicesVirtualInQemuKvm()
		if err != nil {
			return err
		}
	} else if vars.PlatformType == consts.VirtualOpenStack {
