...
func createDevicePluginResource(
	...
	} else if len(p.Spec.NicSelector.PciAddresses) > 0 {
		// Add PciAddresses process logic.
		// For now it is mutually exclusive with Netfilter
		// If both are set, NetFilter will take precedence.

		if len(nodeState.Status.Interfaces) == 0 {
			return nil, fmt.Errorf("node state %s doesn't contain interfaces data", nodeState.Name)
		}
		setPciAddresses(&netDeviceSelectors, p, nodeState)
	}
...
// setPciAddresses retrieves the interfaces of the nodeState based on the provided Node Policy
// and add the PciAddresses of the matched interfaces to netDeviceSelectors.
func setPciAddresses(netDeviceSelectors *dptypes.NetDeviceSelectors,
	p *sriovnetworkv1.SriovNetworkNodePolicy,
	nodeState *sriovnetworkv1.SriovNetworkNodeState) {

	// build a map of PciAddresses for better search
	PciAddressMap := make(map[string]struct{})
	for _, pciAddress := range p.Spec.NicSelector.PciAddresses {
		PciAddressMap[pciAddress] = struct{}{}
	}

	// default search domain is nodeState.Status.Interfaces
	searchVFDomain := false
	// when rootDevices is empty and numVfs is 0,
	// set search domain to nodeState.Status.Interfaces.Vfs
	if len(p.Spec.NicSelector.RootDevices) == 0 &&
		len(p.Spec.NicSelector.PfNames) == 0 &&
		p.Spec.NumVfs == 0 {
		searchVFDomain = true
	}

	// if searchVFDomain = true
	// Loop through the interfaces status to find matched vfs
	if searchVFDomain {
		for _, pf := range nodeState.Status.Interfaces {
			for _, vf := range pf.VFs {
				if _, exists := PciAddressMap[vf.PciAddress]; exists {
					// Found a match, add the VF's PciAddress
					netDeviceSelectors.PciAddresses = sriovnetworkv1.UniqueAppend(netDeviceSelectors.PciAddresses, vf.PciAddress)
				}
			}
		}
		return
	...
	// else,
	// Loop through the interfaces status to find matched pfs
	for _, pf := range nodeState.Status.Interfaces {
		if _, exists := PciAddressMap[pf.PciAddress]; exists {
			// Found a match, add the Interface's PciAddress
			netDeviceSelectors.PciAddresses = sriovnetworkv1.UniqueAppend(netDeviceSelectors.PciAddresses, pf.PciAddress)
		}
	}
}
