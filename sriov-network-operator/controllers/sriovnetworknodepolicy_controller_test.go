...
func TestRenderDevicePluginConfigData(t *testing.T) {
	...
	for _, tc := range table {
		...
		if err := reconciler.Client.Update(context.TODO(), &nodeState); err != nil {
			t.Fatal(err)
		}
...
// test code for renderDevicePluginConfigData
// with PCI addresses field filled in SriovNetworkNodePolicy

// factory function to create test single test case
func newPCITestCase(t *testing.T, name string,
	policySpec sriovnetworkv1.SriovNetworkNodePolicySpec,
	expectedPCIs []string) struct {
	tname       string
	policy      sriovnetworkv1.SriovNetworkNodePolicy
	expResource dptypes.ResourceConfList
} {
	return struct {
		...
		tname:       name,
		policy:      sriovnetworkv1.SriovNetworkNodePolicy{Spec: policySpec},
		expResource: createExpectedResource(t, policySpec.ResourceName, expectedPCIs),
	}
}

// factory function to create expected resource configuration
func createExpectedResource(t *testing.T, resName string, pciAddrs []string) dptypes.ResourceConfList {
	selectors := &dptypes.NetDeviceSelectors{}
	if len(pciAddrs) > 0 {
		selectors.DeviceSelectors = dptypes.DeviceSelectors{
			PciAddresses: pciAddrs,
		}
	}

	return dptypes.ResourceConfList{
		ResourceList: []dptypes.ResourceConfig{
			{
				ResourceName: resName,
				Selectors:    mustMarshallSelector(t, selectors),
			},
		},
	}
}

// createPFPolicy creates a SriovNetworkNodePolicySpec for a Physical Function (PF)
func createPFPolicy(resName string, pciAddrs []string, numVfs int) sriovnetworkv1.SriovNetworkNodePolicySpec {
	return sriovnetworkv1.SriovNetworkNodePolicySpec{
		ResourceName: resName,
		NumVfs:       numVfs,
...
// createVFPolicy creates a SriovNetworkNodePolicySpec for a Virtual Function (VF)
func createVFPolicy(resName string, pciAddrs []string) sriovnetworkv1.SriovNetworkNodePolicySpec {
	return sriovnetworkv1.SriovNetworkNodePolicySpec{
		ResourceName: resName,
		NicSelector: sriovnetworkv1.SriovNetworkNicSelector{
			PciAddresses: pciAddrs,
...
// getPCITestCases returns a slice of test cases for PCI address selection
func getPCITestCases(t *testing.T) []struct {
	tname       string
	policy      sriovnetworkv1.SriovNetworkNodePolicy
	expResource dptypes.ResourceConfList
} {
	return []struct {
		...
		newPCITestCase(t, "testValidPciAddressesForPF",
			createPFPolicy("resourceName", []string{"0000:86:00.0", "0000:86:00.1"}, 1),
			[]string{"0000:86:00.0", "0000:86:00.1"}),

		newPCITestCase(t, "testInvalidPciAddressesForPF",
			createPFPolicy("resourceName", []string{"0000:86:00.0", "0000:86:00.1"}, 0),
			nil),

		newPCITestCase(t, "testValidPciAddressesForVF",
			createVFPolicy("resourceName", []string{"0000:86:00.2", "0000:86:00.3"}),
			[]string{"0000:86:00.2", "0000:86:00.3"}),

		newPCITestCase(t, "testInvalidPciAddressesForVF",
			createVFPolicy("resourceName", []string{"0000:86:00.0", "0000:86:00.1"}),
			nil),
	}
}

// getTestNodeState returns a SriovNetworkNodeState object
// with predefined PCI addresses to match the test cases above.
func getTestNodeState() sriovnetworkv1.SriovNetworkNodeState {
	return sriovnetworkv1.SriovNetworkNodeState{
		ObjectMeta: metav1.ObjectMeta{Name: "node1", Namespace: vars.Namespace},
		Status: sriovnetworkv1.SriovNetworkNodeStateStatus{
			Interfaces: sriovnetworkv1.InterfaceExts{
				sriovnetworkv1.InterfaceExt{
					PciAddress: "0000:86:00.0", // 匹配测试用例中的PCI地址
					Vendor:     "8086",
					Driver:     "i40e",
				},
				sriovnetworkv1.InterfaceExt{
					PciAddress: "0000:86:00.1", // 匹配测试用例中的PCI地址
					Vendor:     "8086",
					Driver:     "i40e",
				},
				sriovnetworkv1.InterfaceExt{
					VFs: []sriovnetworkv1.VirtualFunction{
						{
							PciAddress: "0000:86:00.2", // 匹配测试用例中的PCI地址
							Vendor:     "8086",
							Driver:     "i40e",
						},
						{
							PciAddress: "0000:86:00.3", // 匹配测试用例中的PCI地址
							Vendor:     "8086",
							Driver:     "i40e",
						},
					},
...
// setupTestEnvironment initializes a test SriovNetworkNodePolicyReconciler.
func setupTestEnvironment() SriovNetworkNodePolicyReconciler {

	reconciler := SriovNetworkNodePolicyReconciler{
		FeatureGate: featuregate.New(),
	}
	nodeState := getTestNodeState()
...
// testPciAddressSelector tests the rendering of device plugin configuration data
// based on the PCI addresses specified in the SriovNetworkNodePolicy.
// It checks if the expected resource configuration matches the actual rendered output.
func TestPciAddressSelector(t *testing.T) {
	testTable := getPCITestCases(t)
	reconciler := setupTestEnvironment()
	node := corev1.Node{ObjectMeta: metav1.ObjectMeta{Name: "node1"}}

	for _, tc := range testTable {
		t.Run(tc.tname, func(t *testing.T) {
			policy := tc.policy
			policy.SetNamespace(vars.Namespace)
			policyList := sriovnetworkv1.SriovNetworkNodePolicyList{Items: []sriovnetworkv1.SriovNetworkNodePolicy{tc.policy}}
			resourceList, err := reconciler.renderDevicePluginConfigData(context.TODO(), &policyList, &node)
			if err != nil {
				t.Fatalf("renderDevicePluginConfigData() has failed: %s", err)
			}

			if !cmp.Equal(resourceList, tc.expResource) {
				t.Fatalf("ResourceConfList not as expected: %s", cmp.Diff(resourceList, tc.expResource))
			}
		})
	}
}
